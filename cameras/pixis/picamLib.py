"""
The following script was forked from Daniel Dietze work below the original authors notes
...
class picam():
    ...
    def loadLibrary(self, pathToLib=""):
        ...
        :param str pathToLib: Path to the dynamic link library (optional). If empty, the
        library is loaded using the path given by the environment variabel *PicamRoot*,
         which is normally created by the PICam SDK installer.
    ...
    def getAvailableCameras(self):
        ...
        id_count = piint()
        x = self.status(self.lib.Picam_GetAvailableCameraIDs(ptr(self.camIDs), ptr(id_count)))
        ...
        if id_count.value < 1:
            ...
            self.status(self.lib.Picam_ConnectDemoCamera(model_array[53], serial, ptr(model_ID)))
    ...
    def status(self, err):
        """Checks the return value of a pixis function for any error code. If an error occurred, it prints the error message to stdout.
    ...
    def connect(self, camID=None):
        ...
        :param int camID: Number / index of camera to connect to (optional).
         It is an integer index into a list of valid camera IDs that has been retrieved
         by :py:func:`getAvailableCameras`. If camID is None, this functions connects
         pto the first available camera (default).
        """
        # rint(camID, type(camID), 'camID')
    ...
    def getAvailableParameters(self):
        ...
        for i in range(parameter_count.value):
            ...
            try:
                ...
                param_dict[PicamParameterLookup[parameter_array[i]]] = [readable, constraint]
                print(PicamParameterLookup[parameter_array[i]])
                print(" value access:", readable)
                print(" allowed values:", constraint)

            except Exception as e:
                print(str(e), "Parmaerr")
                pass
        self.lib.Picam_DestroyParameters(parameter_array)
    ...
    def getBuffer(self, address, size):
        ...
        data = ((data.reshape(size, readoutstride)[:, :frames * framestride]).reshape(size, frames, framestride)[:, :,
                :self.totalFrameSize]).reshape(size * frames, self.totalFrameSize).astype(float)
